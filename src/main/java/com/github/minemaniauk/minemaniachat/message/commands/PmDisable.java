...
public class PmDisable implements SimpleCommand {
    @Override
    public void execute(Invocation invocation) {

        String[] args = invocation.arguments();
        if (args.length < 1) {
            invocation.source().sendPlainMessage("Usage: /disablepm <Player|global>");
            return;
        }

        if (invocation.arguments()[0].equals("global")){
            if (MineManiaChat.getInstance().getDataManager().isGlobalPmsEnabled()){
                MineManiaChat.getInstance().getDataManager().globalDisablePms();
                invocation.source().sendMessage(LegacyComponentSerializer.legacyAmpersand().deserialize("&7&l> &cDisabled &7PMs globally"));
                return;
            }
            else {
                invocation.source().sendMessage(LegacyComponentSerializer.legacyAmpersand().deserialize("&c&l> &cPMs already globally disabled"));
                return;
            }
        }
        ...
        if (recipient.isPresent()){
            if (MineManiaChat.getInstance().getDataManager().isPlayerPmFlagEnabled(recipient.get())) {
                MineManiaChat.getInstance().getDataManager().disablePmsUser(recipient.get());
                invocation.source().sendMessage(LegacyComponentSerializer.legacyAmpersand().deserialize("&7&l> &cDisabled &7Pms for " + recipient.get().getUsername()));
            }
            else {
                invocation.source().sendMessage(LegacyComponentSerializer.legacyAmpersand().deserialize("&c&l> &cPMs already disabled for player"));
            }
        }
        else {
